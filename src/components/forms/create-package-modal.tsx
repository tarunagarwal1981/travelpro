...
const predefinedDestinations = [
  { name: 'Golden Triangle', description: 'Delhi - Agra - Jaipur', image: 'https://images.unsplash.com/photo-1564507592333-c60657eea523?w=400&h=300&fit=crop' },
  { name: 'Kerala Backwaters', description: 'Kochi - Alleppey - Kovalam', image: 'https://images.unsplash.com/photo-1602216056096-3b40cc0c9944?w=400&h=300&fit=crop' },
  { name: 'Himalayan Trek', description: 'Manali - Leh - Ladakh', image: 'https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=400&h=300&fit=crop' },
  { name: 'Rajasthan Heritage', description: 'Jaipur - Udaipur - Jodhpur', image: 'https://images.unsplash.com/photo-1477587458883-47145ed94245?w=400&h=300&fit=crop' },
  { name: 'Goa Beaches', description: 'North & South Goa', image: 'https://images.unsplash.com/photo-1512343879784-a960bf40e7f2?w=400&h=300&fit=crop' },
  { name: 'Tamil Nadu Temples', description: 'Chennai - Madurai - Thanjavur', image: 'https://images.unsplash.com/photo-1582510003544-4d00b7f74220?w=400&h=300&fit=crop' }
]
...
const packageTemplates = [
  {
    ...
    color: '#3b82f6',
    templates: ['Heritage Tours', 'Temple Circuits', 'Cultural Immersion', 'Historical Sites']
  },
  {
    category: 'Adventure',
    icon: <Mountain className="icon" />,
    color: '#10b981',
    templates: ['Trekking', 'Mountain Climbing', 'Wildlife Safari', 'River Rafting']
  },
  {
    category: 'Relaxation',
    icon: <Waves className="icon" />,
    color: '#8b5cf6',
    templates: ['Beach Resort', 'Spa Retreat', 'Wellness Tours', 'Luxury Getaway']
  },
  {
    category: 'Spiritual',
    icon: <Heart className="icon" />,
    color: '#f59e0b',
    templates: ['Pilgrimage', 'Meditation Retreat', 'Yoga Tours', 'Sacred Sites']
  }
]
        ...
        return (
          <div className="step-content">
            <div className="step-header">
              ...
              <p>Select a popular destination or create your own custom route</p>
            </div>
            <div className="destinations-grid">
              {predefinedDestinations.map((destination, index) => (
                <div
                  key={index}
                  className={`destination-card ${selectedDestination?.name === destination.name ? 'selected' : ''}`}
            ...
            <div className="category-selection">
              ...
              <div className="categories-grid">
                {packageTemplates.map((template, index) => (
                  <div
                    key={index}
                    className={`category-card ${packageData.category === template.category ? 'selected' : ''}`}
                    onClick={() => handleCategorySelect(template.category)}
                    style={{ borderColor: packageData.category === template.category ? template.color : '#e5e7eb' }}
                  >
                    <div className="category-icon" style={{ color: template.color }}>
                      {template.icon}
                    </div>
              ...
              <div className="form-grid">
                ...
                <div className="form-group">
                  ...
                  <div className="number-input">
                    <button onClick={() => setPackageData({ ...packageData, duration: Math.max(1, packageData.duration - 1) })}>
                      <Minus className="icon" />
                    </button>
                    <span>{packageData.duration}</span>
                    <button onClick={() => setPackageData({ ...packageData, duration: packageData.duration + 1 })}>
                      <Plus className="icon" />
                    </button>
                  </div>
                </div>
                ...
                <div className="form-group">
                  ...
                  <div className="difficulty-buttons">
                    {['Easy', 'Moderate', 'Challenging'].map((level) => (
                      <button
                        key={level}
                        className={`difficulty-btn ${packageData.difficulty === level ? 'active' : ''}`}
                        onClick={() => setPackageData({ ...packageData, difficulty: level })}
                      >
                        {level}
                      </button>
                    ))}
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => setPackageData({
                          ...packageData,
                          groupSize: { ...packageData.groupSize, min: parseInt(e.target.value) || 1 }
                        })}
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => setPackageData({
                          ...packageData,
                          groupSize: { ...packageData.groupSize, max: parseInt(e.target.value) || 1 }
                        })}
                      />
                    </div>
                  </div>
              ...
              <div className="inclusions-section">
                ...
                <div className="items-grid">
                  {sampleInclusions.map((inclusion, index) => (
                    <div
                      key={index}
                      className={`item-card ${packageData.inclusions.includes(inclusion) ? 'selected' : ''}`}
                      onClick={() => toggleInclusion(inclusion)}
                    >
                      <div className="item-checkbox">
                        {packageData.inclusions.includes(inclusion) && <Check className="icon" />}
                      </div>
                      <span>{inclusion}</span>
                    </div>
                  ))}
              ...
              <div className="exclusions-section">
                ...
                <div className="items-grid">
                  {sampleExclusions.map((exclusion, index) => (
                    <div
                      key={index}
                      className={`item-card ${packageData.exclusions.includes(exclusion) ? 'selected' : ''}`}
                      onClick={() => toggleExclusion(exclusion)}
                    >
                      <div className="item-checkbox">
                        {packageData.exclusions.includes(exclusion) && <Check className="icon" />}
                      </div>
                      <span>{exclusion}</span>
                    </div>
                  ))}
                ...
                <div className="form-group">
                  ...
                  <div className="price-input">
                    ...
                    <Input
                      ...
                      onChange={(e) => setPackageData({ ...packageData, originalPrice: parseInt(e.target.value) || 0 })}
                    />
                  </div>
                </div>
              </div>
                ...
                <div className="price-display">
                  ...
                  {packageData.originalPrice > packageData.price && (
                    <Badge className="discount-badge">
                      {Math.round((1 - packageData.price / packageData.originalPrice) * 100)}% OFF
                    </Badge>
                  )}
                </div>
              </div>
                ...
                <div className="preview-content">
                  ...
                  <div className="preview-details">
                    ...
                    <div className="detail-item">
                      ...
                      <span>{packageData.groupSize.min}-{packageData.groupSize.max} travelers</span>
                    </div>
                  </div>
                  <p className="preview-description">{packageData.description}</p>
                </div>
              </div>
            </div>
          </div>
